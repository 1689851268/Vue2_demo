...
export default {
    ...
    methods: {
        // 获取验证码
        getCode() {
            // 简单验证一下，至少填写了手机号，才可以获取验证码
            this.phone &&
                this.$store
                    .dispatch("user/getCode", this.phone)
                    .then(() => {
                        // 自动填写验证码
                        this.code = this.$store.state.user.code; // this.$store.state 为总仓库
                    })
        ...
        async userRegister() {
            ...
            if (success) {
                ...
                this.$store
                    ...
                    .then(() => this.$router.push("/login")) // 成功则跳转到登陆页面
                    .catch((err) => {
                        alert(err);
                        this.clearData();
                    });
        ...
        clearData() {
            ...
            this.code = ""; // 验证码
            this.password = ""; // 密码
            this.checkPassword = ""; // 确认密码
            this.agree = false; // 协议勾选
        },
    },
};
</script>
